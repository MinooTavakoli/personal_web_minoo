...
class CarouselSlider extends Component {
  constructor(props) {
    ...
    this.itemsMargin = this.props.itemsStyle
      ? this.props.itemsStyle.margin
        ...
        : parseInt(
            this.defaultItemsStyle.margin.split(" ")[1].replace("px", "")
          )
      : parseInt(this.defaultItemsStyle.margin.split(" ")[1].replace("px", ""));

    this.slideType = this.props.slideItems
      ? "prop"
      : this.props.slideCpnts
      ? "cpnt"
      : null;
    this.slideCnt =
      this.slideType === "prop"
        ? this.props.slideItems.length
        : this.slideType === "cpnt"
        ? this.props.slideCpnts.length
        : 0;
    this.slideEls =
      this.slideType === "prop"
        ? this.props.slideItems
        : this.slideType === "cpnt"
        ? this.props.slideCpnts
        : null;
  ...
  componentDidMount() {
    this.dragEvent.threshold =
      this.refs.sliderBox.offsetWidth > 100
        ? this.refs.sliderBox.offsetWidth / 5
        : 50;
  }
  ...
  nonImgLoaded(index) {
    ...
    if (this.refs[refIndex]) {
      ...
      if (
        (!this.mannerSetting.circular && index === 1) ||
        (this.mannerSetting.circular && this.slideCnt === 1)
      ) {
  ...
  slideItemHandler(slideEl, index) {
    switch (this.slideType) {
      case "prop":
        let imgElement = slideEl.imgSrc ? (
          <img src={slideEl.imgSrc} />
        ) : (
          <img src={slideEl.imgSrc} />
        );
        let desElement = slideEl.des ? (
          <p style={this.setTextBoxStyle()}>{slideEl.des}</p>
        ) : null;
        ...
        return (
          <div
            ...
            style={{ order: index }}
            onError={() => this.nonImgLoaded(index)}
  ...
  initialSlideCon() {
    ...
    slideCon.addEventListener("transitionend", () => {
    ...
    if (this.mannerSetting.circular && this.slideCnt !== 1) {
      ...
      this.slidingManner.slidesTotalWidth =
        sum + this.slideCnt * this.itemsMargin * 2;
      ...
      slideCon.style.transition =
        "transform " + this.mannerSetting.duration + " ease";
      this.slidingManner.initialMovement =
        (this.refs.sliderBox.offsetWidth - this.imgsWidth[1]) / 2 +
        this.itemsMargin +
        this.imgsWidth[1];
    }

    // slideCon.style.transition = 'transform ' + this.mannerSetting.duration + ' ease';
    slideCon.style.transform =
      "translateX(-" + this.slidingManner.initialMovement + "px)";
  ...
  itemsReorder() {
    for (let i = 0; i < this.slideCnt * 3; i++) {
      if (i < this.slideCnt) {
        this.refs["ref" + (i - this.slideCnt)].style.order =
          this.slidingManner.slideOrders[this.slidingManner.cycle][i];
      } else {
        this.refs["ref" + (i - this.slideCnt + 1)].style.order =
          this.slidingManner.slideOrders[this.slidingManner.cycle][i];
      }
    }
  }
      ...
      if (!this.slidingManner.sliding) {
        ...
        if (direction > 0) {
          ...
          slideCon.style.transform =
            ...
            "px)";
          this.slidingManner.movement =
            this.slidingManner.movement + this.slidingManner.slidesTotalWidth;
        }
  ...
  moveSlide(direction) {
    let expectLoadedCnt = this.mannerSetting.circular
      ? this.slideCnt * 3
      : this.slideCnt;
    if (this.loadedCnt === expectLoadedCnt) {
      ...
      if (this.mannerSetting.circular) {
        ...
        if (direction === 1) {
          ...
          this.slidingManner.movement =
            this.slidingManner.movement + singleMovement;
          ...
          if (this.state.currentSlide === this.slideCnt) {
            this.setState(
              ...
              () => {
                slideCon.style.transform =
                  "translateX(-" + this.slidingManner.movement + "px)";
                this.slidingManner.cycle =
                  this.slidingManner.cycle === 2
                    ? 0
                    : this.slidingManner.cycle + 1;
                this.itemsReorder();
        ...
        slideCon.style.transition =
          "transform " + this.mannerSetting.duration + " ease"; // for after dragging
  ...
  calMovement(direction) {
    ...
    let movement =
      this.imgsWidth[this.state.currentSlide] + this.itemsMargin * 2;
    return movement;
  }
  ...
  handleTouchMove(e) {
    if (this.touchEvent.beingTouched) {
      this.slidingManner.sliding = true;
      this.touchEvent.touchMovement =
        this.touchEvent.touchStartX - e.targetTouches[0].clientX;
      let dragMovement =
        this.slidingManner.movement + this.touchEvent.touchMovement;
  ...
  handleTouchEnd(e) {
    if (this.touchEvent.beingTouched) {
      if (
        Math.abs(this.touchEvent.touchStartX - e.changedTouches[0].clientX) > 20
      ) {
        this.slidingManner.sliding = false;
        if (!this.slidingManner.sliding) {
          if (
            Math.abs(this.touchEvent.touchMovement) > this.dragEvent.threshold
          ) {
  ...
  setSlideConHeight() {
    if (this.props.itemsStyle) {
      return this.props.itemsStyle.height
        ? { height: this.props.itemsStyle.height }
        : { height: this.defaultSlideConStyle.height };
    } else {
      ...
      if (this.props.buttonSetting.style) {
        if (this.props.buttonSetting.style.right) {
          btnStyle = Object.assign(
            {},
            this.defaultButtonIconStyle,
            this.props.buttonSetting.style.right
          );
        }
      }
    }
      ...
      if (this.props.buttonSetting.style) {
        if (this.props.buttonSetting.style.left) {
          btnStyle = Object.assign(
            {},
            this.defaultButtonIconStyle,
            this.props.buttonSetting.style.left
          );
        }
      }
    }
  ...
  setHoverEvent() {
    let hoverEffect;
    if (this.props.buttonSetting) {
      hoverEffect = this.props.buttonSetting.hoverEvent
        ? this.props.buttonSetting.hoverEvent
        : this.buttonManner.hoverEvent;
      if (hoverEffect) {
        return {
          opacity: 0,
        };
  ...
  render() {
    ...
    if (this.slideCnt) {
      if (this.accEleSetting.button) {
        ...
        lButtonIcon = this.props.lBtnCpnt ? (
          this.props.lBtnCpnt
        ) : (
          <div
            className="arrowBtn previous"
            style={this.setLeftButtonStyle()}
          />
        );
        rButtonIcon = this.props.rBtnCpnt ? (
          this.props.rBtnCpnt
        ) : (
        ...
        if (buttonSetting.separate) {
          leftButton = (
            <div
              className="buttonWrapper left"
              style={this.setHoverEvent()}
          ...
          buttons = (
            <div
              className="buttonSet"
              style={this.allocateButtonSet(buttonSetting)}
            >
              <div
              ...
              <div
                ...
                style={this.setHoverEvent()}
                onClick={() => this.handleButtonClick(1)}
                style={{ position: "absolute", right: "-4px", top: "105px" }}
              >
                {rButtonIcon}
              </div>
            </div>
      ...
      if (this.mannerSetting.circular && this.slideCnt > 1) {
        ...
        items = this.slideEls.map((slideEl, index) =>
          this.slideItemHandler(slideEl, index - this.slideCnt)
        );
        items = items.concat(
          this.slideEls.map((slideEl, index) =>
            this.slideItemHandler(slideEl, index + 1)
          )
        );
        items = items.concat(
          this.slideEls.map((slideEl, index) =>
            this.slideItemHandler(slideEl, index + 1 + this.slideCnt)
          )
        );
      } else {
        if (this.accEleSetting.flag) {
          ...
          sflag = (
            <div
              className="flag"
              ref="sflag"
              style={{ background: "transparent", order: 0 }}
              key={"sflag"}
            />
          );
          eflag = (
            <div
              className="flag"
              ref="eflag"
              style={{ background: "transparent", order: this.slideCnt + 1 }}
              key={"eflag"}
            />
          );
        }
      ...
      if (this.accEleSetting.dots) {
        ...
        for (let i = 0; i < this.slideCnt; i++) {
          ...
          if (i + 1 === this.state.currentSlide) {
            dot = <div key={"dot" + i} style={this.setCurrentDotStyle()} />;
          } else {
            dot = <div key={"dot" + i} style={this.setDotStyle()} />;
          }
          dots.push(dot);
        }
        dotsSet = (
          <div className="dotsSet" style={this.allocateDotsSet(dotsSetting)}>
            {dots}
          </div>
        );
      }
        ...
        if (this.accEleSetting.dots) {
          renderSlideItems = (
            <div className="sliderSet">
              {buttonSetting.outOfBox
                ? buttonSetting.separate
                  ? leftButton
                  : null
                : null}
              <div
